...
const startRepair = (repairId: string) => {
  // props.service.repairs.find((value, index) => {
  //   if (value.id === repairId) {
  //     alert(value.employee_name);
  //     return true;
  //   }
  // });
...
const onSubmit = () => {
  let countServiceTechnician = 0;
  props.service.repairs.forEach((value) => {
    if (value.employee_id === null) countServiceTechnician++;
  });

  if (countServiceTechnician > 0) {
    alertDialog.message =
      "Untuk melakukan proses service pada kendaraan silahkan pilih mekanik terlebih dahulu.";
...
<template>
  ...
  <div class="p-4">
    ...
    <AlertDialog
      ...
      alert-type="error"
      :messages="alertDialog.message"
      @closed="alertDialog.show = false"
    />
  </div>
</template>
