...
import ReportSaleCodeBox from "@/Components/Report/ReportSaleCodeBox.vue";
...
const saleTable = ref<InstanceType<typeof DataTable> | null>(null);
const columns: ColumnDef<ISale>[] = [
  {
    ...
    cell: ({ row }) => h(ReportSaleCodeBox, { sale: row.original }),
  },
  {
...
const totalMoneySaleTransaction = computed(() => {
  return props.sales.data.reduce(
...
const printTransaction = () => {
  ...
  router.get(route("backoffice.report.sale-print"), url2.value, {
    onError: (error) => console.log(error),
    onStart: () => (isLoading.value = true),
    onFinish: () => (isLoading.value = false),
  });
...
const exportTransaction = () => {
  ...
  if (
    ...
    window.open(route("backoffice.report.sale-export"));
  }
};
  ...
  <div>
    <div class="flex items-center divide-x divide-gray-300 p-2">
      <div class="flex items-center px-4 gap-4 text-primary">
        ...
        <h1 class="text-lg font-semibold">Laporan Transaksi Penjualan</h1>
      </div>
    </div>
    <HeaderInformation>
        ...
        <template #filter>
          <div class="inline-flex items-center gap-2">
            ...
            <div class="space-x-1">
              ...
              <Button variant="default" @click="getSales(1)" type="button">
                Cari Transaksi
              </Button>
            </div>
            <div class="space-x-1">
